...
namespace BS.Output.Paint
{
  public class OutputAddIn: V3.OutputAddIn<Output>
  {
    ...
    {
      get { return "Open screenshots in Paint for direct editing."; }
    }
    ...
    protected async override Task<V3.SendResult> Send(IWin32Window Owner, Output Output, V3.ImageData ImageData)
    {
      try
      {
        ...
        string applicationPath = Environment.GetFolderPath(Environment.SpecialFolder.System) + "\\mspaint.exe";

        if (!File.Exists(applicationPath))
        {
          return new V3.SendResult(V3.Result.Failed, "Microsoft Paint is not installed.");
        }
